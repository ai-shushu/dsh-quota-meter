...
if ! command -v "$DSH_CMD" >/dev/null 2>&1; then
  ...
  echo "      在 deepseek-harness 仓库根执行：pnpm dsh plugin --profile $PROFILE remove quota-meter" >&2
  exit 1
fi

echo "==> 卸载 quota-meter bundle → profile: $PROFILE"
"$DSH_CMD" plugin --profile "$PROFILE" remove quota-meter
