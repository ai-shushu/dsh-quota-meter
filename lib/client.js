...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			...
			// 一个卡片圆角半径），即只覆盖卡片顶部两个拐弯之间的直线段；
			// 条本身不带圆角（两端直角，终止于卡片弧线起点）
			trackWrap: { display: "flex", justifyContent: "center", width: "100%" },
			track: { position: "relative", width: "calc(min(var(--dsh-composer-card-max-width), 100% - 32px) - 44px)", height: "5px", borderRadius: "0px", background: "var(--dsw-alias-bg-layer-2)", overflow: "hidden", boxShadow: "inset 0 0 0 1px var(--dsw-alias-border-l2-darkmode-thin)" },
		...
		function QuotaMeter(props) {
			...
			// 填充：右对齐；脉冲时整体闪白高亮；渐变左浅右深 + 微弱辉光；两端直角
			var fillBg = pulse
