...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			...
			muted: { opacity: 0.75 },
			// 细进度条轨道：宽度与输入框卡片一致，5px 高，两端 22px 圆角跟随卡片
			trackWrap: { display: "flex", justifyContent: "center", width: "100%" },
			track: { position: "relative", width: "min(var(--dsh-composer-card-max-width), calc(100% - 32px))", height: "5px", borderRadius: "22px", background: "var(--dsw-alias-bg-layer-2)", overflow: "hidden", boxShadow: "inset 0 0 0 1px var(--dsw-alias-border-l2-darkmode-thin)" },
