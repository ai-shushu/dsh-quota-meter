...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			...
			modal: { width: 520, maxWidth: "calc(100vw - 32px)", maxHeight: "72vh", overflowY: "auto", padding: "16px 18px", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 14, background: "var(--dsw-alias-bg-overlay)", color: "var(--dsw-alias-label-primary)", boxShadow: "0 12px 40px rgba(0,0,0,.28)", fontSize: 12 },
			...
			modelBlock: { padding: "6px 0", borderBottom: "1px solid var(--dsw-alias-border-l1)" },
			modelRowHead: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 },
			...
			priceInput: { width: 56, padding: "2px 6px", border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 6, background: "var(--dsw-alias-bg-layer-1)", color: "var(--dsw-alias-label-primary)", fontSize: 12, outline: "none", textAlign: "right" },
			addRow: { display: "flex", alignItems: "center", gap: "6px", padding: "8px 0", flexWrap: "wrap" },
			nameInput: { flex: "0 1 110px", minWidth: 56, padding: "2px 6px", border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 6, background: "var(--dsw-alias-bg-layer-1)", color: "var(--dsw-alias-label-primary)", fontSize: 12, outline: "none" },
			providerInput: { flex: "0 1 70px", minWidth: 0, padding: "2px 6px", border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 6, background: "var(--dsw-alias-bg-layer-1)", color: "var(--dsw-alias-label-primary)", fontSize: 12, outline: "none" },
		...
		function QuotaMeter(props) {
			...
			var addHook = react.useState({ name: "", provider: "", miss: "", hit: "", out: "", tod: false, peakMiss: "", peakHit: "", peakOut: "" });
			var add = addHook[0];
			...
			var addModel = function () {
				...
				setAdd({ name: "", provider: "", miss: "", hit: "", out: "", tod: false, peakMiss: "", peakHit: "", peakOut: "" });
			};
			...
			var pricesModal = function () {
				...
				return react.createElement("div", { style: S.modalOverlay, onClick: closePrices },
					react.createElement("div", { style: S.modal, onClick: function (e) { e.stopPropagation(); } },
						...
						react.createElement("div", { style: S.modalSub }, "单位 " + (pricesDraft.unit || "¥") + " / 每 " + (pricesDraft.per || "1M") + " tokens · 未命中 / 命中 / 输出；峰谷模型分「空闲 / 高峰」两组（DeepSeek 北京时间 9-12、14-18 为高峰）"),
							...
							return react.createElement("div", { key: name, style: S.modelBlock },
								react.createElement("div", { style: S.modelRowHead },
									react.createElement("div", { style: S.modelMeta },
										react.createElement("div", { style: S.modelName }, name + (tod ? " · 峰谷" : "")),
