	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && !document.getElementById("quota-meter-flow")) {
			...
			flowStyle.textContent = "@keyframes quota-meter-flow { from { background-position: 0 0, 0 0; } to { background-position: 48px 0, 0 0; } }"
				+ " @keyframes quota-meter-badge { 0% { opacity: 0; transform: translateY(-3px); } 12% { opacity: 1; transform: translateY(0); } 78% { opacity: 1; } 100% { opacity: 0; } }"
				...
				+ " @keyframes quota-meter-burn { 0%, 100% { opacity: .45; transform: translate(-50%, -50%) scale(.75); } 50% { opacity: 1; transform: translate(-50%, -50%) scale(1.2); } }";
		...
		var S = {
			...
			// 文字行：已花~徽标 左对齐，改额度/价格 右对齐（限宽居中，不超出对话框）
			row: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: "10px", maxWidth: "var(--dsh-composer-card-max-width)", margin: "0 auto", padding: "5px 16px 7px", fontSize: "12px", lineHeight: "1.4", color: "var(--dsw-alias-label-secondary)", whiteSpace: "nowrap" },
			...
			// 消耗徽标：本次烧掉的金额，弹出后淡出（与额度大小无关，大额度也可见）
			badge: { flex: "none", display: "inline-block", padding: "0 6px", borderRadius: "999px", background: "var(--dsw-alias-bg-module-platform)", color: "var(--dsw-alias-label-tertiary)", fontSize: "11px", lineHeight: "16px", fontVariantNumeric: "tabular-nums", animation: "quota-meter-badge 1.2s ease forwards" },
		...
		function QuotaMeter(props) {
			...
			var textNode = danger
				...
				: react.createElement("span", { style: S.text },
					react.createElement("strong", { style: S.strong }, spentText),
					" / " + unit + fmt(quota) + " · 剩余 ",
					// 数字滚动（B）：消耗瞬间剩余数字跳动落定（key 变化重放）
					react.createElement("span", { key: badgeTick, style: { display: "inline-block", fontVariantNumeric: "tabular-nums", animation: badgeTick > 0 ? "quota-meter-num .35s ease" : undefined } },
						unit + fmt(remaining)
					)
				);
			...
			// 请求中亮点（燃烧）：请求发起时在剩余段左端持续脉动，请求结束消失
			...
			var burnStyle = {
				...
				borderRadius: "50%",
				background: "radial-gradient(circle, rgba(255,255,255,.95), rgba(255,255,255,.25) 60%, transparent)",
				animation: "quota-meter-burn 1.1s ease-in-out infinite",
				pointerEvents: "none"
			};
			...
			return react.createElement("div", { style: Object.assign({}, S.wrap, { marginBottom: "-8px" }) },
				...
				react.createElement("div", { style: S.trackWrap },
					react.createElement("div", { style: trackStyle },
						...
						!danger && inflight && react.createElement("div", { style: burnStyle })
					)
				)
			);
		}
