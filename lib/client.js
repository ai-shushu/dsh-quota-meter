...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			...
			missingIgnore: { border: "none", background: "none", padding: 0, color: "var(--dsw-static-deepseek-500)", fontSize: 12, cursor: "pointer" },
					...
					react.createElement("div", { style: S.dialog, onClick: function (e) { e.stopPropagation(); } },
						...
						react.createElement("div", { style: S.body },
							...
							(function () {
								...
								var missing = used.filter(function (m) {
									return !(pricesDraft.models && pricesDraft.models[m]) && ignored.indexOf(m) < 0;
								});
								if (missing.length === 0) return null;
								return react.createElement("div", { style: S.missingBox },
									react.createElement("span", null, "检测到未配置价格的模型（当前按 fallback 计价）："),
									missing.map(function (m) {
										return react.createElement("span", { key: m, style: { display: "inline-flex", alignItems: "center", gap: 8 } },
											react.createElement("button", { style: S.missingBtn, onClick: function () { setAdd(Object.assign({}, add, { name: m })); } }, m + " · 补配"),
											react.createElement("button", { style: S.missingIgnore, title: "不再提示该模型", onClick: function () { ignoreModel(m); } }, "忽略")
										);
									})
								);
							})(),
