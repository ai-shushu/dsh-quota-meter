...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			...
			track: { position: "relative", width: "calc(min(var(--dsh-composer-card-max-width), 100% - 32px) - 44px)", height: "5px", borderRadius: "4px", background: "transparent", overflow: "hidden" },
		...
		function QuotaMeter(props) {
			...
			// 填充：右对齐；两端 4px 圆头（无延伸物、无截面）；脉冲闪白；
			// 渐变左浅右深 + 辉光
			var fillBg = pulse
