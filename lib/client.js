...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			...
			// 弹层顶部未配价模型提示区
			missingBox: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 10, padding: "6px 8px", borderRadius: 8, background: "var(--dsw-alias-state-warn-tertiary)", color: "var(--dsw-alias-state-warn-primary)", fontSize: 11 },
			missingBtn: { border: "none", background: "none", padding: 0, color: "var(--dsw-alias-state-warn-primary)", fontSize: 11, cursor: "pointer", textDecoration: "underline", textUnderlineOffset: "2px" },
			// 模型条目
		...
		function QuotaMeter(props) {
			...
			// 细条：黑色剩余（右对齐倒退）+ 浅灰消耗痕迹；danger 整条红。
			...
			var scanBandStyle = {
				...
				animation: "quota-meter-scan 2.6s ease-out infinite"
			};
