		...
		function QuotaMeter(props) {
			...
			var ignoreNewModel = async function (m) {
				...
				setSyncResult(function (prev) {
					if (!prev) return prev;
					var newModels = (prev.newModels || []).filter(function (x) { return x !== m; });
			...
			var addModel = function () {
				...
				setPricesDraft(function (prev) {
					...
					return Object.assign({}, prev, {
						models: models,
						fallback: prev.fallback || name,
						// 补配即配价：同步脱离已忽略/Plan 标记（否则"已忽略/Plan"组里仍残留）
						ignored: (prev.ignored || []).filter(function (m) { return m !== name; }),
						plans: (prev.plans || []).filter(function (m) { return m !== name; })
					});
				});
				...
				setCatalogUnpriced(function (prev) {
					return prev
						.map(function (g) { return Object.assign({}, g, { models: (g.models || []).filter(function (m) { return m !== name; }) }); })
			...
			var removeModel = function (name) {
				...
				if (!prev || !prev.models || !prev.models[name]) return;
				var models = Object.assign({}, prev.models);
				delete models[name];
					...
					if (!syncResult.hasChanges) {
						...
						syncBody = react.createElement("div", { style: S.syncWrap },
							...
							(syncResult.changes || []).map(function (c) {
								return react.createElement("div", { key: c.model, style: S.syncRow },
									react.createElement("span", { style: S.syncModel }, c.model),
							...
							entries.map(function (entry) {
								...
								return react.createElement("div", { key: name, style: S.modelCard },
									react.createElement("div", { style: S.modelHead, title: isOpen ? "折叠" : "展开", onClick: function () { toggleModel(name); } },
										...
										react.createElement("span", { style: S.modelName, title: name }, name),
										// 三列固定宽度徽标列：峰谷 / 状态 / 平台——缺位也渲染（留空），保证跨卡对齐
										react.createElement("span", { style: Object.assign({}, S.pill, tod ? {} : { borderColor: "transparent" }) }, tod ? "峰谷" : ""),
										react.createElement("span", { style: Object.assign({}, S.todState, (tod && todStatus && todStatus[name]) ? (todStatus[name].peak ? S.todStatePeak : S.todStateFree) : {}) },
											(tod && todStatus && todStatus[name]) ? (todStatus[name].peak ? "● 高峰中" : "● 空闲中") : ""),
										react.createElement("span", { style: Object.assign({}, S.modelProvider, m.provider ? {} : { background: "transparent" }) }, m.provider || ""),
										react.createElement("div", { style: { flex: 1 } }),
