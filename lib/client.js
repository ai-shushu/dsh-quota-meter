...
// UI 完全对齐 harness 设计语言：
//   - 输入框上方 = 官方 dock 卡片条（Todo/Goal/Queue 家族：36px、radius 12、
//     border-l1、bg-specific-tip），内含消耗文字 + 4px 中性进度条 + 操作按钮
//   - 进度条 = 黑色剩余（label-primary）+ 浅灰轨道（border-l3），右对齐倒退；
//     耗尽时整条变红（state-error-primary）；无彩色渐变、无流光
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			// ── 输入框上方的 dock 卡片条（对齐 Todo/GoalBar 家族）──
			bar: {
			...
			// 消耗文字（GoalBar objective 规格：13/20 dimmed）
			text: { flex: "none", minWidth: 0, fontSize: "13px", lineHeight: "20px", color: "var(--dsw-alias-label-primary-dimmed)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" },
			strong: { color: "var(--dsw-alias-label-primary)", fontWeight: 500, fontVariantNumeric: "tabular-nums" },
			// 4px 进度条（ContextMeter 规格：浅灰轨道 + 中性填充）
			track: { position: "relative", flex: "1 1 auto", minWidth: 60, height: "4px", borderRadius: "999px", background: "var(--dsw-alias-border-l3)", overflow: "hidden" },
			// 黑色剩余（右对齐倒退）；danger 由调用处覆盖为红
			fill: { position: "absolute", right: 0, top: 0, bottom: 0, borderRadius: "999px", background: "var(--dsw-alias-label-primary)", transition: "width .4s ease" },
			// 设置控件（未设额度时的行内表单）
			set: { display: "inline-flex", alignItems: "center", gap: "6px", flex: "none" },
			// 表单输入（GoalBar objectiveInput 规格：26px、r6、border-l2、bg-base）
		...
		function QuotaMeter(props) {
			...
			if (state === null) {
				return react.createElement("div", { style: S.bar },
					react.createElement("span", { style: S.text }, "额度加载中…")
				);
			}
			...
			if (state.quota === null || state.quota <= 0) {
				return react.createElement("div", { style: S.bar },
					...
					react.createElement("button", { style: S.btnSm, onClick: openPrices }, "价格"),
					pricesModal()
				);
			}
			...
			var textStyle = { flex: "none", minWidth: 0, fontSize: "13px", lineHeight: "20px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" };
			var textNode = danger
				? react.createElement("span", { style: Object.assign({}, textStyle, { color: "var(--dsw-alias-state-error-primary)", fontWeight: 500 }) },
					"额度已用完 " + unit + fmt(spent) + " / " + unit + fmt(quota)
				)
				: react.createElement("span", { style: Object.assign({}, textStyle, { color: "var(--dsw-alias-label-primary-dimmed)" }) },
					react.createElement("strong", { style: S.strong }, spentText),
					" / " + unit + fmt(quota) + " · 剩余 " + unit + fmt(remaining)
				);

			// 进度条：黑色剩余（右对齐倒退），danger 整条红
			var fillStyle = Object.assign({}, S.fill, { width: (ratio * 100) + "%", background: danger ? "var(--dsw-alias-state-error-primary)" : "var(--dsw-alias-label-primary)" });
			var trackStyle = Object.assign({}, S.track);
			if (danger) trackStyle.background = "color-mix(in srgb, var(--dsw-alias-state-error-primary) 30%, transparent)";
			...
			return react.createElement("div", { style: S.bar },
				...
				editing
					...
					: react.createElement("button", { style: S.btnSm, onClick: function () { setDraft(fmt(quota)); setEditing(true); } }, "改额度"),
				react.createElement("button", { style: S.btnSm, onClick: openPrices }, "价格"),
				pricesModal()
			);
		}
