	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && !document.getElementById("quota-meter-flow")) {
			...
			flowStyle.textContent = "@keyframes quota-meter-flow { from { background-position: 0 0, 0 0; } to { background-position: 48px 0, 0 0; } }"
				+ " @keyframes quota-meter-erase { from { transform: translateX(0); opacity: .9; } to { transform: translateX(-48px); opacity: 0; } }"
				+ " @keyframes quota-meter-num { from { color: var(--dsw-alias-label-tertiary); } to { color: inherit; } }";
		...
		function QuotaMeter(props) {
			...
			// 消耗消退动效：检测 spent 增量 → 触发擦除扫过 + 数字联动
			var prevSpent = react.useRef(null);
			var eraseHook = react.useState(0);
			var eraseTick = eraseHook[0];
			var setEraseTick = eraseHook[1];
			...
			react.useEffect(function () {
				...
				if (prevSpent.current !== null && s > prevSpent.current) {
					setEraseTick(function (n) { return n + 1; });
				}
				prevSpent.current = s;
			}, [state]);
			...
			var textNode = danger
				...
				: react.createElement("span", { style: S.text },
					react.createElement("strong", { style: S.strong }, spentText),
					" / " + unit + fmt(quota) + " · 剩余 ",
					// 数字联动（D）：消耗瞬间剩余数字闪灰再恢复
					react.createElement("span", { key: eraseTick, style: { fontVariantNumeric: "tabular-nums", animation: eraseTick > 0 ? "quota-meter-num .35s ease" : undefined } },
						unit + fmt(remaining)
					)
				);
			...
			// 擦除带（A）：消耗瞬间从 fill 左端向左扫过并淡出（key 变化重放动画）
			var eraseStyle = {
				position: "absolute", top: 0, bottom: 0,
				left: ((1 - ratio) * 100) + "%",
				width: "48px",
				background: "linear-gradient(90deg, transparent, rgba(255,255,255,.55), transparent)",
				animation: "quota-meter-erase .4s ease-out forwards",
				pointerEvents: "none"
			};
			...
			return react.createElement("div", { style: Object.assign({}, S.wrap, { marginBottom: "-8px" }) },
				...
				react.createElement("div", { style: S.trackWrap },
					react.createElement("div", { style: trackStyle },
						...
						!danger && eraseTick > 0 && react.createElement("div", { key: eraseTick, style: eraseStyle })
					)
				)
			);
		}
