...
//   - 价格弹层内置「从DS官方更新价格」（右上角）：抓 DeepSeek 官方定价页（zh-cn）做 diff，
//     确认后应用合并入库（GET/POST /quota/prices/check|apply，host 侧同步）
//   - 耗尽提示 = 官方 Modal 风格对话框
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var S = {
			...
			// 弹层卡片峰谷状态点 = 固定 64px 列（高峰中 = 琥珀 / 空闲中 = 绿；非峰谷模型留空保持列位）
			todState: { flex: "none", width: "64px", boxSizing: "border-box", textAlign: "center", fontSize: "11px", lineHeight: "16px", padding: "1px 0", borderRadius: "4px" },
			...
			// 模型名占固定比例列（45% + 均分余量，超长省略号）——保证后续徽标列起始 x 全卡一致
			modelName: { flex: "1 1 45%", minWidth: 0, fontSize: "14px", lineHeight: "22px", fontWeight: 500, color: "var(--dsw-alias-label-primary)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" },
			// 峰谷胶囊 = 固定 36px 列（非峰谷模型内容留空、边框透明，列位保持）
			pill: { flex: "none", width: "36px", boxSizing: "border-box", textAlign: "center", fontSize: "11px", lineHeight: "16px", padding: "1px 0", border: "1px solid var(--dsw-alias-border-l3)", borderRadius: "4px", color: "var(--dsw-alias-label-secondary)" },
			// 平台芯片 = 固定 96px 列（长平台名省略号；窄窗口可收缩）
			modelProvider: { flex: "0 1 96px", minWidth: 0, width: "96px", boxSizing: "border-box", textAlign: "center", fontSize: "11px", lineHeight: "16px", padding: "1px 0", borderRadius: "4px", background: "var(--dsw-alias-bg-module-platform)", color: "var(--dsw-alias-label-secondary)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" },
			...
			// 官方价格同步区：结果卡（diff/一致/失败）+ 逐模型变更行
			syncWrap: { display: "flex", flexDirection: "column", gap: "6px", padding: "8px 10px", borderRadius: "8px", border: "1px solid var(--dsw-alias-border-l2)" },
			syncMeta: { fontSize: "11px", lineHeight: "16px", color: "var(--dsw-alias-label-tertiary)", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
			syncNotice: { fontSize: "12px", lineHeight: "18px", color: "var(--dsw-alias-state-warn-primary)", fontWeight: 600 },
			syncRow: { display: "flex", flexDirection: "column", gap: 2, padding: "5px 0", borderTop: "1px solid var(--dsw-alias-border-l2)" },
			syncModel: { fontSize: "12px", lineHeight: "18px", fontWeight: 600, color: "var(--dsw-alias-label-primary)" },
			syncDiffs: { fontSize: "11px", lineHeight: "16px", color: "var(--dsw-alias-label-secondary)", fontVariantNumeric: "tabular-nums" },
		...
		// ── v3 (provider, model) 引用工具（与 host 半部一致）──
		// provider 空 = 任意厂家全局匹配；\u0000 连接保证 model 内含 '/' 也无歧义
		function refKey(ref) {
			var r = ref && typeof ref === "object" ? ref : { provider: "", model: String(ref || "") };
		...
		function listHas(list, provider, model) {
			...
			var p = provider || "";
			return list.some(function (x) { return x && x.model === model && (!x.provider || x.provider === "" || x.provider === p); });
		}
		function entryExists(models, provider, model) {
			if (!model || !models) return false;
			if (provider) return !!(models[provider] && models[provider][model]);
			return Object.keys(models).some(function (p) { return !!(models[p] && models[p][model]); });
		}
		function displayRef(r) {
			var prov = (r && r.provider) || "";
			var m = (r && r.model) || "";
			return prov ? prov + " / " + m : m;
		}
		...
		function QuotaMeter(props) {
			...
			// 完整目录的 provider id 列表（「添加模型」平台下拉的受控枚举，防误操作重复）
			var catalogGroupsHook = react.useState([]);
			var catalogGroups = catalogGroupsHook[0];
			var setCatalogGroups = catalogGroupsHook[1];
			// 目录段展开状态（默认折叠）
			...
			// 官方价格同步状态：idle | checking | done | error
			var syncStateHook = react.useState("idle");
			var syncState = syncStateHook[0];
			var setSyncState = syncStateHook[1];
			var syncResultHook = react.useState(null);
			var syncResult = syncResultHook[0];
			var setSyncResult = syncResultHook[1];
			var syncErrHook = react.useState("");
			var syncErr = syncErrHook[0];
			var setSyncErr = syncErrHook[1];
			...
			var loadPricesData = async function () {
				try {
					...
					if (r && r.catalogGroups) setCatalogGroups(r.catalogGroups);
			...
			var savePrices = async function () {
				...
				try {
					...
					if (r && r.ok) {
						...
						// 重新拉取目录未配价：刚保存配价的模型不再出现（下次打开也一致）
						await loadPricesData();
						setPricesOpen(false);
					} else {
			...
			// ---- 官方价格同步：检查（只读 diff）/ 应用（确认后合并） ----
			var checkOfficial = async function () {
				setSyncErr("");
				setSyncState("checking");
				try {
					var r = await window.fetch("/quota/prices/check").then(function (x) { return x.json(); });
					if (r && r.ok) {
						setSyncResult(r);
						setSyncState("done");
					} else {
						setSyncErr((r && r.reason) || "检查失败");
						setSyncState("error");
					}
				} catch (err) { setSyncErr("网络错误"); setSyncState("error"); }
			};

			var applyOfficial = async function () {
				setSyncErr("");
				try {
					var r = await window.fetch("/quota/prices/apply", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({})
					}).then(function (x) { return x.json(); });
					if (r && r.ok) {
						// 应用成功：刷新弹层草稿 + 目录未配价清单（新模型已配价，不再提示）
						await loadPricesData();
						setSyncState("idle");
						setSyncResult(null);
					} else {
						setSyncErr((r && r.reason) || "应用失败");
					}
				} catch (err) { setSyncErr("应用失败"); }
			};

			// 新增模型行内「忽略」：官方页有、但用户不想要的模型 → 立即忽略并退出 diff 列表
			var ignoreNewModel = async function (m) {
				await ignoreModel(m);
				setSyncResult(function (prev) {
					if (!prev) return prev;
					var newModels = (prev.newModels || []).filter(function (x) { return refKey(x) !== refKey(m); });
					return Object.assign({}, prev, {
						newModels: newModels,
						hasChanges: newModels.length > 0 || (prev.changes || []).length > 0
					});
				});
			};

			// 通用价格字段更新：key 形如 "inputMiss" 或 "default.inputMiss"/"peak.inputMiss"
			var setModelField = function (provider, name, key, value) {
				setPricesDraft(function (prev) {
					var models = Object.assign({}, prev.models);
					var group = Object.assign({}, models[provider] || {});
					var entry = Object.assign({}, group[name]);
					var prices = Object.assign({}, entry.prices);
					var parts = key.split(".");
					if (parts.length === 2) {
						var g = Object.assign({}, prices[parts[0]] || {});
						g[parts[1]] = value;
						prices[parts[0]] = g;
					} else {
			...
			var setTodField = function (provider, name, key, value) {
				setPricesDraft(function (prev) {
					var models = Object.assign({}, prev.models);
					var group = Object.assign({}, models[provider] || {});
					var entry = Object.assign({}, group[name]);
			...
			var addModel = function () {
				var provider = add.provider.trim();
				var name = add.name.trim();
				if (!name) { setPricesErr("请填写模型名"); return; }
				if (!provider) { setPricesErr("请选择平台（厂家）"); return; }
				if (provider.indexOf("/") >= 0) { setPricesErr("平台名不能包含 /"); return; }
				if ((pricesDraft.models[provider] || {})[name]) { setPricesErr("该平台下已存在模型 " + name); return; }
				var ref = { provider: provider, model: name };
				var key = refKey(ref);
				setPricesDraft(function (prev) {
					var models = Object.assign({}, prev.models);
					var group = Object.assign({}, models[provider] || {});
					...
					return Object.assign({}, prev, {
						models: models,
						fallback: (prev.fallback && prev.fallback.model) ? prev.fallback : ref,
						// 补配即配价：同步脱离已忽略/Plan 标记（否则"已忽略/Plan"组里仍残留）
						ignored: (prev.ignored || []).filter(function (m) { return refKey(m) !== key; }),
						plans: (prev.plans || []).filter(function (m) { return refKey(m) !== key; })
					});
				});
				// 补配成功：目录段立即移除该模型（已用段随 pricesDraft 更新自动消失；
				// 目录段是打开弹层时的快照，不本地移除会残留到下次重开）
				setCatalogUnpriced(function (prev) {
					return prev
						.map(function (g) { return Object.assign({}, g, { models: (g.models || []).filter(function (m) { return !(m.provider === provider && m.id === name); }) }); })
			...
			var removeModel = function (provider, name) {
				// 删除 = 立即持久化（与「忽略/Plan」一致）：只改草稿会在 check/刷新时
				// 与服务端不一致——服务端模型还在 → check 显示"与官方一致" → 刷新又回来
				var prev = pricesDraft;
				if (!prev || !prev.models || !prev.models[provider] || !prev.models[provider][name]) return;
				var models = Object.assign({}, prev.models);
				var group = Object.assign({}, models[provider]);
				delete group[name];
				if (Object.keys(group).length === 0) delete models[provider];
				else models[provider] = group;
				if (Object.keys(models).length === 0) { setPricesErr("至少保留一个模型"); return; }
				// fallback 若指向被删模型 → 改指第一组第一条
				var fb = prev.fallback;
				var fallback = fb && fb.model === name && (fb.provider === "" || fb.provider === provider)
					? (function () { for (var p in models) { var g = models[p]; for (var m in g) return { provider: p, model: m }; } return fb; })()
					: fb;
				var key = refKey({ provider: provider, model: name });
				var nextDraft = Object.assign({}, prev, {
					models: models,
					fallback: fallback,
					// 删除即彻底移除：同步清理 ignored/plans 里的残留标记
					ignored: (prev.ignored || []).filter(function (m) { return refKey(m) !== key; }),
					plans: (prev.plans || []).filter(function (m) { return refKey(m) !== key; })
				});
				setPricesDraft(nextDraft);
				setCollapsed(function (prev2) {
					if (!prev2[key]) return prev2;
					var next = Object.assign({}, prev2);
					delete next[key];
					return next;
				});
				// 立即写盘；成功后以服务端结果为权威刷新草稿（失败的模型会回来并提示）
				window.fetch("/quota/prices", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify(nextDraft)
				}).then(function (x) { return x.json(); }).then(function (r) {
					if (r && r.ok) return loadPricesData();
					setPricesErr((r && r.reason) || "删除失败");
				}).catch(function () { setPricesErr("删除失败"); });
			};
			...
			// ref = { provider, model }（provider 空 = 全局）
			var ignoreModel = async function (ref) {
				setPricesErr("");
				try {
					var r = await window.fetch("/quota/prices", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ ignore: ref })
					}).then(function (x) { return x.json(); });
			...
			var planModel = async function (ref) {
				setPricesErr("");
				try {
					var r = await window.fetch("/quota/prices", {
						method: "POST",
			...
			// 取消忽略（该模型回到未配价提示）
			var unignoreModel = async function (ref) {
				setPricesErr("");
				try {
					var r = await window.fetch("/quota/prices", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ unignore: ref })
					}).then(function (x) { return x.json(); });
					if (r && r.prices) await loadPricesData();
			...
			// 取消 Plan（该模型回到未配价提示）
			var unplanModel = async function (ref) {
				setPricesErr("");
				try {
					var r = await window.fetch("/quota/prices", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ unplan: ref })
					}).then(function (x) { return x.json(); });
					if (r && r.prices) await loadPricesData();
			...
			// 三档价格网格（未命中 / 命中 / 输出）
			var priceGrid = function (provider, name, group) {
				var keyOf = function (f) { return group ? group + "." + f : f; };
				var entry = pricesDraft.models[provider][name];
				var p = group ? (entry.prices[group] || {}) : entry.prices;
				return react.createElement("div", { style: S.priceGrid },
					priceField("未命中", p.inputMiss, function (e) { setModelField(provider, name, keyOf("inputMiss"), e.target.value); }),
					priceField("命中", p.inputHit, function (e) { setModelField(provider, name, keyOf("inputHit"), e.target.value); }),
					priceField("输出", p.output, function (e) { setModelField(provider, name, keyOf("output"), e.target.value); })
				);
			};

			var pricesModal = function () {
				if (!pricesOpen || pricesDraft === null) return null;
				// v3 嵌套 → 平铺卡片列表：{ provider, name, entry, key }，key = provider\0name
				var cards = [];
				Object.keys(pricesDraft.models || {}).forEach(function (provider) {
					var group = pricesDraft.models[provider] || {};
					Object.keys(group).forEach(function (name) {
						cards.push({ provider: provider, name: name, m: group[name], key: refKey({ provider: provider, model: name }) });
					});
				});
				// 平台受控枚举（厂家唯一性治理）：完整目录厂家 + 价目表已有厂家，去重排序；
				// 目录取不到（冷会话/失败）时退回自由输入
				var providerOptions = [];
				(catalogGroups || []).forEach(function (p) { if (p && providerOptions.indexOf(p) < 0) providerOptions.push(p); });
				Object.keys(pricesDraft.models || {}).forEach(function (p) { if (p && providerOptions.indexOf(p) < 0) providerOptions.push(p); });
				providerOptions.sort();
				// 官方价格同步：按钮在 header 右上角，结果（一致 / diff+应用 / 失败）在 body
				var syncMeta = (syncState === "done" && syncResult)
					? react.createElement("span", { style: S.syncMeta },
						"来源 " + (syncResult.source || "") + " · " + (syncResult.fetchedAt ? new Date(syncResult.fetchedAt).toLocaleString() : ""))
					: null;
				var syncBody = null;
				if (syncState === "done" && syncResult) {
					if (!syncResult.hasChanges) {
						syncBody = react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 2 } },
							react.createElement("div", { style: { fontSize: "12px", lineHeight: "18px", color: "var(--dsw-alias-label-secondary)" } }, "与官方价目表一致，无需更新"),
							syncMeta
						);
					} else {
						syncBody = react.createElement("div", { style: S.syncWrap },
							react.createElement("div", { style: S.syncNotice }, "发现官方价格更新："),
							(syncResult.newModels || []).length > 0
								? react.createElement(react.Fragment, null,
									react.createElement("div", { style: { fontSize: "11px", lineHeight: "16px", color: "var(--dsw-alias-label-secondary)" } }, "官方新增模型（可忽略，不再提示）："),
									(syncResult.newModels || []).map(function (m) {
										return react.createElement("div", { key: refKey(m), style: S.missingRow },
											react.createElement("span", { style: S.missingModel, title: displayRef(m) }, displayRef(m)),
											react.createElement("button", { style: S.missingIgnore, title: "忽略该模型，不再提示、也不入库", onClick: function () { ignoreNewModel(m); } }, "忽略")
										);
									})
								)
								: null,
							(syncResult.changes || []).map(function (c) {
								return react.createElement("div", { key: refKey(c.model), style: S.syncRow },
									react.createElement("span", { style: S.syncModel }, displayRef(c.model)),
									react.createElement("span", { style: S.syncDiffs }, c.changes.join("；"))
								);
							}),
							syncMeta,
							react.createElement("div", { style: { display: "flex", gap: 8, paddingTop: 6 } },
								react.createElement("button", { style: S.btnPrimarySm, onClick: applyOfficial }, "应用更新"),
								react.createElement("button", { style: S.btnOutlineSm, onClick: function () { setSyncState("idle"); setSyncResult(null); } }, "暂不应用")
							)
						);
					}
				} else if (syncState === "error") {
					syncBody = react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 2 } },
						react.createElement("div", { style: { fontSize: "12px", lineHeight: "18px", color: "var(--dsw-alias-state-error-primary)" } },
							"检查失败：" + (syncErr || "") + "（保留当前价）"),
						syncMeta
					);
				}
				// header 右上角按钮：空闲 = 品牌蓝胶囊；检查中 = 灰且禁用
				var syncButton = react.createElement("button", {
					style: syncState === "checking" ? S.btnSm : S.btnPrimarySm,
					onClick: checkOfficial,
					disabled: syncState === "checking"
				}, syncState === "checking" ? "检查中…" : "从DS官方更新价格");
				...
				var todRow = function (provider, name, m) {
					var tod = Object.assign({ tz: "Asia/Shanghai", peak: [[9, 12], [14, 18]] }, m.tod || {});
					var peak = tod.peak || [];
					var w0 = peak[0] || [9, 12];
					var w1 = peak[1] || [14, 18];
					var numInput = function (key, v) {
						return react.createElement("input", { style: S.tinyInput, value: String(v), onChange: function (e) { setTodField(provider, name, key, e.target.value); } });
					};
					return react.createElement("div", { style: S.todRow },
						...
						react.createElement("span", { style: S.todSep }, "–"),
						numInput("1e", w1[1]),
						react.createElement("input", { style: S.tzInput, placeholder: "时区", value: tod.tz || "", onChange: function (e) { setTodField(provider, name, "tz", e.target.value); } })
					);
				};
					...
					react.createElement("div", { style: S.dialog, onClick: function (e) { e.stopPropagation(); } },
						...
						react.createElement("div", { style: S.body },
							...
							(function () {
								...
								var models = pricesDraft.models || {};
								var usedMissing = used.filter(function (u) {
									var ref = (u && typeof u === "object") ? u : { provider: "", model: String(u || "") };
									var m = ref.model;
									if (!m) return false;
									if (entryExists(models, ref.provider || "", m)) return false;
									if (listHas(pricesDraft.ignored, ref.provider || "", m)) return false;
									if (listHas(pricesDraft.plans, ref.provider || "", m)) return false;
									return true;
								});
								...
								var actionRow = function (m, provider) {
									var key = refKey({ provider: provider || "", model: m });
									var label = provider ? provider + " / " + m : m;
									return react.createElement("div", { key: key, style: S.missingRow },
										react.createElement("span", { style: S.missingModel, title: label }, label),
										...
										react.createElement("button", { style: S.missingIgnore, title: "按默认价粗略计费，不再提示", onClick: function () { ignoreModel({ provider: provider || "", model: m }); } }, "忽略"),
										react.createElement("button", { style: S.missingPlan, title: "套餐模型，调用不产生费用", onClick: function () { planModel({ provider: provider || "", model: m }); } }, "Plan")
									);
								};
								return react.createElement("div", { style: S.missingWrap },
									...
									usedMissing.length > 0
										? react.createElement("div", { style: S.missingBox },
											...
											usedMissing.map(function (u) { return actionRow((u && typeof u === "object") ? u.model : String(u || ""), (u && typeof u === "object") ? (u.provider || "") : ""); })
										)
										: null,
									catalogCount > 0
												...
												? react.createElement("div", { style: S.catalogList },
													catalogGroups.map(function (g) {
														return react.createElement("div", { key: g.provider, style: S.catalogGroup },
															...
															(g.models || []).map(function (m) { return actionRow(m.id, m.provider); })
														);
													})
												)
												: null
							...
							(function () {
								...
								var markedRow = function (m, isPlan) {
									var ref = (m && typeof m === "object") ? m : { provider: "", model: String(m || "") };
									return react.createElement("div", { key: refKey(ref), style: S.missingRow },
										react.createElement("span", { style: S.missingModel, title: displayRef(ref) }, displayRef(ref)),
										react.createElement("button", { style: S.missingFix, title: "按你填写的价格计费", onClick: function () { setAdd(Object.assign({}, add, { name: ref.model, provider: ref.provider || add.provider })); } }, "补配"),
										react.createElement("button", { style: S.markedCancel, title: isPlan ? "取消 Plan，回到未配价提示" : "取消忽略，回到未配价提示", onClick: function () { if (isPlan) unplanModel(ref); else unignoreModel(ref); } }, isPlan ? "取消 Plan" : "取消忽略")
									);
								};
							...
							cards.map(function (card) {
								...
								var isOpen = !!collapsed[card.key];
								return react.createElement("div", { key: card.key, style: S.modelCard },
									react.createElement("div", { style: S.modelHead, title: isOpen ? "折叠" : "展开", onClick: function () { toggleModel(card.key); } },
										react.createElement("span", { style: S.chevron }, isOpen ? "▾" : "▸"),
										react.createElement("span", { style: S.modelName, title: provider + " / " + name }, name),
										// 三列固定宽度徽标列：峰谷 / 状态 / 平台——缺位也渲染（留空），保证跨卡对齐
										react.createElement("span", { style: Object.assign({}, S.pill, tod ? {} : { borderColor: "transparent" }) }, tod ? "峰谷" : ""),
										react.createElement("span", { style: Object.assign({}, S.todState, (tod && todStatus && todStatus[card.key]) ? (todStatus[card.key].peak ? S.todStatePeak : S.todStateFree) : {}) },
											(tod && todStatus && todStatus[card.key]) ? (todStatus[card.key].peak ? "● 高峰中" : "● 空闲中") : ""),
										react.createElement("span", { style: Object.assign({}, S.modelProvider, provider ? {} : { background: "transparent" }) }, provider),
										react.createElement("div", { style: { flex: 1 } }),
										react.createElement("button", { style: S.btnSm, onClick: function (e) { e.stopPropagation(); removeModel(provider, name); } }, "删除")
									),
									isOpen
										? (tod
											? react.createElement(react.Fragment, null,
												react.createElement("span", { style: S.groupLabel }, "空闲"),
												priceGrid(provider, name, "default"),
												react.createElement("span", { style: S.groupLabel }, "高峰"),
												priceGrid(provider, name, "peak"),
												todRow(provider, name, m)
											)
											: priceGrid(provider, name, null))
										: null
								);
							}),
							react.createElement("div", { style: S.editorCard },
								...
								react.createElement("div", { style: S.field },
									react.createElement("span", { style: S.fieldLabel }, "平台（厂家）"),
									providerOptions.length > 0
										? react.createElement("select", { style: S.textInput, value: add.provider, onChange: function (e) { setAdd(Object.assign({}, add, { provider: e.target.value })); } },
											react.createElement("option", { value: "", key: "" }, "选择平台…"),
											providerOptions.map(function (p) { return react.createElement("option", { value: p, key: p }, p); })
										)
			...
			var currentLabel = currentProvider ? currentProvider + " / " + currentModel : currentModel;
			var unpricedBadge = state.currentUnpriced
				? react.createElement("button", { style: S.unpricedBadge, onClick: openPrices, title: "当前模型 " + currentLabel + " 未配置价格，按 fallback 计价" }, "⚠ " + currentLabel + " 未配价")
				: null;
