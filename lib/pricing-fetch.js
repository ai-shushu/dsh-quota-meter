...
// 官方解析结果 vs 当前价目表 → { hasChanges, changes:[{model,changes:[]}], newModels:[...] }
// 只读：不修改任何输入。
export function diffPrices(current, officialModels, officialTod) {
  const changes = []
  const newModels = []
  for (const [name, o] of Object.entries(officialModels)) {
    const cur = current.models && current.models[name]
    if (!cur) { newModels.push(name); continue }
    const diffs = []
    ...
    if (diffs.length > 0) changes.push({ model: name, changes: diffs })
  }
...
// only：可选模型白名单（缺省 = 全部官方模型）。
export function applyOfficial(current, officialModels, officialTod, only) {
  const models = {}
  for (const [name, m] of Object.entries(current.models || {})) models[name] = JSON.parse(JSON.stringify(m))
  for (const [name, o] of Object.entries(officialModels)) {
    if (only && only.indexOf(name) < 0) continue
    let entry = models[name]
    if (!entry) entry = { provider: 'deepseek', pricing: 'per-token-tod', tod: {}, prices: {} }
