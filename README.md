...
价格弹层右上角「**从DS官方更新价格**」：抓 DeepSeek 官方定价页（[中文版](https://api-docs.deepseek.com/zh-cn/quick_start/pricing)，¥/每 1M tokens，含高峰/空闲两档）→ 与当前价目表 diff → 弹层内展示变更明细（旧→新 + 新增模型）→ **确认后**一键应用并持久化。
